...
Person-matching is exact-name-only, so two different people sharing a name look like the same contact. The product decision: surface this as a warning rather than blocking or auto-disambiguating — the review screen shows the match and gives the user a `forceNew` escape hatch to save as a distinct person instead of merging. Re-evaluate the match live against current name field input (not just the one-shot AI-extracted name), since the name is user-editable before saving.
